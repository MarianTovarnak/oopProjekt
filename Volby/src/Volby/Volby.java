...
public class Volby {
	...
	private ArrayList<Pstrana> pstrany;
	...
	public void import_pstrany() {
	     ...
	     while (true) {
            ...
            String[] parts = line.split(";");
            Pstrana novaStrana = new Pstrana(Integer.valueOf(parts[0]), parts[1]);
            
            for (int i = pc_obcan; i < (pc_obcan+8) && i < obcania.size(); i++) {
            	Obcan pom = obcania.get(i);
            	if(pom.jePlnolety()) {
            		Kandidat k = new Kandidat(pom.getIdObcan(), pom.getMeno(), pom.getPriezvisko());
	...
	public int overenie_volica(int login_id) {
		...
		return -2; // ak nie je zozname obcanov
	}
