...
public class VolbyGUI extends Application {
    ...
    private void hlasovanie(Stage primaryStage) {
        ...
        loginButton.setOnAction(e -> {
        	try { // ak namiesto cisla za pismeno
    ...
    private String zoznam_p_stran(Stage primaryStage, TextArea textArea) {
         ...
         Label labelO = new Label("Prihlásený: "+volby.getPrihlaseny().getInfo());
         ...
         layout.getChildren().addAll(labelO, label, stranyListView, label2, buttonsBox);
    ...
    private void zakladnaAnalyza() {
        ...
        if (poradieStrd != null && !poradieStrd.isEmpty()) {
            textArea.appendText("Poradie strán podľa počtu získaných hlasov:\n");
    ...
    private void savetofile() {
        try {
            ...
            out.close();
            fileOut.close();
            appendToTextArea("Dáta boli uložené do súboru 'volby.ser'.");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void loadfromFile() {
        try {
            ...
            in.close();
            fileIn.close();
            appendToTextArea("Dáta boli načítané zo súboru 'volby.ser'.");
